...
if (isset($_POST["addtolist"]) || isset($_POST["addandfill"])) {
    ...
    if ($flag) {
        ...
        $report_insert = "INSERT INTO `reports`(`report_id`, `report_laboratory`, `report_patient`, `report_details`, `report_auth`) VALUES (NULL, '$lab_id', '$pid', '$report', '$auth')";
        if (mysqli_query($con, $report_insert)) {
            ...
            if (isset($_POST["addtolist"])) {
                ...
                $_SESSION["smsg"][] = "Report Added";
                header("location:fill_report.php?report=" . $report_sid);
            }
        } else {
            $_SESSION["amsg"][] = "Error";
            header("location:create_report.php");
    ...
    <form method="POST" action="create_report.php">
        <div class="row">
            ...
            <div class="col-12 pt-4 text-center">
                <button type="submit" name="addtolist" id="addtolist" class="btn btn-success mr-3" <?php echo $disabled; ?>><i class="fa fa-plus-circle"></i> Add to List</button>
                <button type="submit" name="addandfill" id="addandfill" class="btn btn-success mr-3" <?php echo $disabled; ?>><i class="fa fa-pencil-square-o"></i> Add and Fill</button>
            </div>
        </div>
    </form>
</main>
<script>
    $(document).ready(function() {
        $("#fetch").click(function() {
            ...
            $.ajax({
                method: 'POST',
                url: '<?php echo $lab_url . "laboratory/"; ?>api.php',
                data: {
                    get_info: 1,
                    pid: my_pid,
                    dob: my_dob
                ...
                success: function(res) {
                    ...
                    if (res.status == 'success') {
                        ...
                        $("#addandfill").prop('disabled', false);
                        if (res.report > 0) {
                            $.confirm({
                                title: 'Panding Reports Found',
                                content: res.report + ' Panding Reports Found For Patient, Want to Proceed With New Report or Fill The Exist Reports?',
                                type: 'blue',
                                typeAnimated: true,
                                buttons: {
                                    close: {
                                    ...
                                    ViewReports: {
                                        ...
                                        btnClass: 'btn-blue',
                                        action: function() {
                                            location.href = "<?php echo $lab_url . 'laboratory/fill_report.php'; ?>?pid=" + my_pid + "&dob=" + my_dob;
                                        }
                                    }
                                }
                            });
